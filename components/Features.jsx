...
const BRAND_ICONS = [Search, ShieldCheck, BarChart3, MessageSquare, Star, RefreshCw]
const CREATOR_ICONS = [Package, ShieldCheck, Trophy, CalendarDays, Users, TrendingUp]
    ...
    <section id="fitur" className="bg-gray-50 py-20 px-4">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-14">
          <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">{f.title}</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-8">
            ...
            <div className="space-y-6">
              {f.brand.map((item, i) => (
                <FeatureCard key={i} Icon={BRAND_ICONS[i]} title={item.title} desc={item.desc} />
              ))}
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-8">
            <div className="flex items-center gap-3 mb-8">
              <div className="w-10 h-10 bg-[#C9A96E] rounded-xl flex items-center justify-center">
                <Clapperboard size={18} className="text-white" strokeWidth={1.5} />
              </div>
              <div>
                <div className="font-bold text-gray-900">{f.creatorTitle}</div>
                <div className="text-xs text-gray-500">{f.creatorSub}</div>
              </div>
            </div>
            <div className="space-y-6">
              {f.creator.map((item, i) => (
                <FeatureCard key={i} Icon={CREATOR_ICONS[i]} title={item.title} desc={item.desc} />
              ))}
            </div>
          </div>
        </div>
